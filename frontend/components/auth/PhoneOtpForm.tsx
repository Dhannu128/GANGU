...
import { requestOtp, verifyOtp, socialSignIn, type SocialProvider } from '@/lib/api'
import { ArrowRight, Phone, Loader, ShieldCheck, ChevronLeft, AlertCircle, KeyRound, Lock, Sparkles } from 'lucide-react'
...
export default function PhoneOtpForm({ mode }: PhoneOtpFormProps) {
  ...
  const { signIn, pushToast } = useGANGUStore()
  ...
  const handleSocial = async (provider: SocialProvider) => {
    ...
    try {
      const res = await socialSignIn(provider, language)
      if (res.success) {
        signIn(
          {
            id: res.user.id,
        ...
        pushToast({
          variant: 'success',
          title: `Welcome${res.user.isNewUser ? '' : ' back'}, ${res.user.name.split(' ')[0]}!`,
          description: provider === 'google' ? 'Signed in with Google.' : 'Signed in via WhatsApp.',
        })
        router.push('/app')
      }
    } catch (err: any) {
    ...
    return (
      <form onSubmit={handleRequest} className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-slate-600 uppercase tracking-widest font-bold">or continue with</span>
          <div className="divider-line flex-1" />
        </div>
  ...
  return (
    <form onSubmit={handleVerify} className="space-y-5">
      ...
      <div>
        ...
        {devHint && (
          <p className="text-[11px] text-slate-500 mt-3 text-center inline-flex items-center justify-center gap-1.5 w-full">
            <Sparkles className="w-3 h-3 text-amber-400/70" />
            <span className="opacity-70">{devHint}</span>
          </p>
        )}
      </div>
...
function TrustStrip() {
  const items = [
    ...
    { icon: KeyRound,     label: 'Delete anytime' },
  ]
  return (
    <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 pt-1">
      {items.map((item, i) => {
        ...
        return (
          <span key={i} className="inline-flex items-center gap-1.5 text-[11px] text-slate-500 uppercase tracking-[0.14em] font-bold">
...
function WhatsAppIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden fill="#25D366">
